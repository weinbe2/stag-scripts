...
if (!(-d "$path/$direc"."/spectrum2/stoch_hv"))
{
        my $command = "mkdir $path/$direc"."/spectrum2/stoch_hv";
        `$command`;
}
...
my $a_path = "$path/$direc"."/dmix/dmix*";
...
open(my $pbpfile, ">$path/$direc"."/spectrum2/stoch_hv/PBPPART_HV.dat");
open(my $connfile, ">$path/$direc"."/spectrum2/stoch_hv/SPECTRUM_HV.dat");
open(my $timefile, ">$path/$direc"."/spectrum2/stoch_hv/times.dat");
