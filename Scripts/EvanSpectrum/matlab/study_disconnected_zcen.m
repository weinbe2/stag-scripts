...
% 2015-04-10 Updated to have same interface as study_connected_new.m.
%function study_disconnected_zcen(fname, blocksize, diag, fold, just_d, tmax_in)
function study_disconnected_zcen(fname, state, noerrors, blockval, tmin_min, tmin_max, tmax)
	% Load ensemble info.
	%[fl_l, fl_h, parse_Ns, parse_Nt, beta, m_l, m_h] = load_info_file(fname);
    [fl_l, fl_h, parse_Ns, parse_Nt, beta, m_l, m_h] = load_info_file(fname);
	fl_flavor = fl_l/4; % Staggered counting.
    
	if (strcmp(state, 'sg_stoch_zcen'))
		state = 'sg_stoch';
	elseif (strcmp(state, 'dc_stoch_zcen'))
		state = 'dc_stoch';
	else
		error('study_disconnected_zcen.m expects only sg_stoch_zcen or dc_stoch_zcen!');
	end
	...
	% To do: update with the study_connected_new.m way to get guesses.
